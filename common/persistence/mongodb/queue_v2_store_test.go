...
func TestQueueV2RangeDeleteIsTransactionalAndVersioned(t *testing.T) {
	db := newFakeQueueV2Database(t)
	mongoClient := &fakeMongoClient{}
	store, err := NewQueueV2Store(db, mongoClient, config.MongoDB{}, log.NewTestLogger(), metrics.NoopMetricsHandler, true)
	require.NoError(t, err)

	queueType := persistence.QueueV2Type(3)
	queueName := "queue-range-delete"
	queueProto := &persistencespb.Queue{
		Partitions: map[int32]*persistencespb.QueuePartition{
			defaultQueueV2Partition: {MinMessageId: persistence.FirstQueueMessageID},
		},
	}
	payload, err := proto.Marshal(queueProto)
	require.NoError(t, err)

	metadataCol := db.collections[collectionQueueV2Metadata]
	metadataCol.enqueueFind(&queueV2MetadataDocument{
		ID:            queueV2MetadataDocID(queueType, queueName),
		QueueType:     int32(queueType),
		QueueName:     queueName,
		Version:       7,
		Payload:       payload,
		Encoding:      enumspb.ENCODING_TYPE_PROTO3.String(),
		LastMessageID: persistence.FirstQueueMessageID + 4,
	})
	metadataCol.enqueueUpdateResult(&mongo.UpdateResult{MatchedCount: 1})

	response, err := store.RangeDeleteMessages(context.Background(), &persistence.InternalRangeDeleteMessagesRequest{
		QueueType: queueType,
		QueueName: queueName,
	...
	require.NoError(t, err)
	require.Equal(t, int64(2), response.MessagesDeleted)
	require.Len(t, mongoClient.sessions, 1)
	require.Equal(t, 1, mongoClient.sessions[0].withTransactionCalls)
	require.Len(t, db.collections[collectionQueueV2Messages].deleteManyFilters, 1)

	require.Len(t, metadataCol.updateFilters, 1)
	updateFilter, ok := metadataCol.updateFilters[0].(bson.M)
	require.True(t, ok)
	require.Equal(t, int64(7), updateFilter["version"])
	update, ok := metadataCol.updateUpdates[0].(bson.M)
	require.True(t, ok)
	increments, ok := update["$inc"].(bson.M)
	require.True(t, ok)
	require.Equal(t, int64(1), increments["version"])
}
