...
func matchingRoutingFilter(namespaceID, taskQueue string, taskType enumspb.TaskQueueType, subqueue int) bson.M {
	return bson.M{
		"namespace_id": namespaceID,
		"task_queue":   taskQueue,
		"task_type":    int32(taskType),
		"subqueue":     subqueue,
	}
}

func taskQueueFilter(namespaceID, taskQueue string, taskType enumspb.TaskQueueType, subqueue int, fairnessEnabled bool) bson.M {
	filter := matchingRoutingFilter(namespaceID, taskQueue, taskType, subqueue)
	filter["_id"] = taskQueueDocID(namespaceID, taskQueue, taskType, subqueue, fairnessEnabled)
	return filter
}
...
func (s *taskStore) ensureIndexes(ctx context.Context) error {
	...
	// The lookup index is not unique because different versions of the same queue can exist.
...
func (s *taskStore) GetTaskQueue(
	...
	filter := taskQueueFilter(request.NamespaceID, request.TaskQueue, request.TaskType, persistence.SubqueueZero, s.fairnessEnabled)
...
func (s *taskStore) UpdateTaskQueue(
	...
	filter := taskQueueFilter(request.NamespaceID, request.TaskQueue, request.TaskType, persistence.SubqueueZero, s.fairnessEnabled)
	filter["range_id"] = request.PrevRangeID
...
func (s *taskStore) DeleteTaskQueue(
	...
	filter := taskQueueFilter(key.NamespaceID, key.TaskQueueName, key.TaskQueueType, persistence.SubqueueZero, s.fairnessEnabled)
	filter["range_id"] = request.RangeID
...
func (s *taskStore) CreateTasks(
	...
	_, txErr := sess.WithTransaction(ctx, func(sessCtx context.Context) (interface{}, error) {
		...
		queueFilter := taskQueueFilter(request.NamespaceID, request.TaskQueue, request.TaskType, persistence.SubqueueZero, s.fairnessEnabled)
		queueFilter["_id"] = queueID
		queueFilter["range_id"] = request.RangeID
		...
		if err != nil {
			return nil, newMongoUnavailableErrorf(err, "CreateTasks: failed to update queue: %v", err)
		}
		if res.MatchedCount == 0 {
		...
		if err != nil {
			...
			return nil, newMongoUnavailableErrorf(err, "CreateTasks: batch insert failed: %v", err)
		}
...
func (s *taskStore) GetTasks(
	...
	filter := matchingRoutingFilter(request.NamespaceID, request.TaskQueue, request.TaskType, request.Subqueue)
...
func (s *taskStore) CompleteTasksLessThan(
	...
	if request.Limit <= 0 {
		return 0, serviceerror.NewInvalidArgument("complete tasks limit must be positive")
	}

	filter := matchingRoutingFilter(request.NamespaceID, request.TaskQueueName, request.TaskType, request.Subqueue)
	...
	sortSpec := bson.D{{Key: "task_id", Value: 1}}
	if s.fairnessEnabled {
		sortSpec = bson.D{{Key: "task_pass", Value: 1}, {Key: "task_id", Value: 1}}
	}
	cursor, err := s.db.Collection(collectionTasks).Find(
		ctx,
		filter,
		options.Find().
	...
	if err != nil {
		return 0, serviceerror.NewUnavailablef("CompleteTasksLessThan failed to select tasks: %v", err)
	}
	defer func() { _ = cursor.Close(ctx) }()
	...
	for cursor.Next(ctx) {
		...
		if err := cursor.Decode(&doc); err != nil {
			return 0, serviceerror.NewUnavailablef("CompleteTasksLessThan failed to decode task: %v", err)
		}
		taskIDs = append(taskIDs, doc.ID)
	}
	if err := cursor.Err(); err != nil {
		return 0, serviceerror.NewUnavailablef("CompleteTasksLessThan failed to iterate tasks: %v", err)
	}
	if len(taskIDs) == 0 {
		return 0, nil
	}

	deleteFilter := matchingRoutingFilter(request.NamespaceID, request.TaskQueueName, request.TaskType, request.Subqueue)
	deleteFilter["_id"] = bson.M{"$in": taskIDs}
	res, err := s.db.Collection(collectionTasks).DeleteMany(ctx, deleteFilter)
	if err != nil {
		return 0, serviceerror.NewUnavailablef("CompleteTasksLessThan failed to delete tasks: %v", err)
	}
...
func (s *taskStore) applyUserDataUpdate(ctx context.Context, namespaceID, taskQueue string, update *persistence.InternalSingleTaskQueueUserDataUpdate) error {
	...
	if update.Version == 0 {
		...
		if err != nil {
			...
			return newMongoUnavailableErrorf(err, "UpdateTaskQueueUserData insert failed: %v", err)
		}
	...
	if err != nil {
		return newMongoUnavailableErrorf(err, "UpdateTaskQueueUserData failed: %v", err)
	}
...
func (s *taskStore) applyBuildIDUpdates(ctx context.Context, namespaceID, taskQueue string, update *persistence.InternalSingleTaskQueueUserDataUpdate) error {
	...
	for _, buildID := range update.BuildIdsAdded {
		...
		if err != nil {
			...
			return newMongoUnavailableErrorf(err, "UpdateTaskQueueUserData build id insert failed: %v", err)
		}
	}
	...
	for _, buildID := range update.BuildIdsRemoved {
		...
		if err != nil {
			return newMongoUnavailableErrorf(err, "UpdateTaskQueueUserData build id delete failed: %v", err)
		}
	}
