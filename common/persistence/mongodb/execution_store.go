...
func (s *executionStore) insertHistoryTaskDoc(
	...
	case taskspkg.CategoryTypeImmediate:
		...
		if _, err := col.InsertOne(ctx, doc); err != nil {
			...
			return newMongoUnavailableErrorf(err, "failed to insert immediate task: %v", err)
		}
	case taskspkg.CategoryTypeScheduled:
		...
		if _, err := col.InsertOne(ctx, doc); err != nil {
			...
			return newMongoUnavailableErrorf(err, "failed to insert scheduled task: %v", err)
		}
	default:
...
func (s *executionStore) loadExecutionDocument(ctx context.Context, shardID int32, namespaceID, workflowID, runID string) (*executionDocument, error) {
	filter := bson.M{"_id": executionDocID(namespaceID, workflowID, runID), "shard_id": shardID}
	var doc executionDocument
	if err := s.executionsCol.FindOne(ctx, filter).Decode(&doc); err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			return nil, nil
		}
		return nil, newMongoUnavailableErrorf(err, "failed to load execution document: %v", err)
	}
	return &doc, nil
}
...
func (s *executionStore) applyMutationToExecutionDocument(
	...
	if mutation.ClearBufferedEvents {
		...
		doc.BufferedEvents = append(doc.BufferedEvents, dataBlobToDoc(mutation.NewBufferedEvents))
	}
...
func (s *executionStore) updateCurrentExecutionForMutation(
	...
	if err != nil {
		return newMongoUnavailableErrorf(err, "failed to update current execution document: %v", err)
	}
	if result.MatchedCount == 0 {
		existing, loadErr := s.loadCurrentExecution(ctx, shardID, namespaceID, workflowID, archetypeID)
		if loadErr != nil {
			return loadErr
		}
...
func (s *executionStore) updateCurrentExecutionForSnapshot(
	...
	return s.updateCurrentExecutionForMutation(ctx, shardID, namespaceID, workflowID, archetypeID, snapshot.ExecutionState, snapshot.LastWriteVersion, snapshot.DBRecordVersion)
}

func (s *executionStore) assertNotCurrentExecution(ctx context.Context, shardID int32, namespaceID, workflowID string, archetypeID chasm.ArchetypeID, runID string) error {
	current, err := s.loadCurrentExecution(ctx, shardID, namespaceID, workflowID, archetypeID)
	if err != nil {
		return err
	}
...
func (s *executionStore) applyWorkflowMutation(
	...
	doc, err := s.loadExecutionDocument(ctx, shardID, mutation.NamespaceID, mutation.WorkflowID, stateProto.GetRunId())
	if err != nil {
		return nil, err
	}
	if doc == nil {
	...
	if err != nil {
		return nil, newMongoUnavailableErrorf(err, "failed to update execution document: %v", err)
	}
	if result.MatchedCount == 0 {
...
func (s *executionStore) applyUpdateWorkflowExecution(
	...
	if request.Mode == persistence.UpdateWorkflowModeUpdateCurrent {
		current, err := s.loadCurrentExecution(ctx, request.ShardID, mutation.NamespaceID, mutation.WorkflowID, request.ArchetypeID)
		if err != nil {
			return err
		}
	...
	case persistence.UpdateWorkflowModeBypassCurrent:
		if err := s.assertNotCurrentExecution(ctx, request.ShardID, mutation.NamespaceID, mutation.WorkflowID, request.ArchetypeID, stateProto.GetRunId()); err != nil {
			return err
		}
	case persistence.UpdateWorkflowModeUpdateCurrent:
		if newState != nil {
			...
			if err := s.updateCurrentExecutionForMutation(ctx, request.ShardID, mutation.NamespaceID, mutation.WorkflowID, request.ArchetypeID, stateProto, mutation.LastWriteVersion, mutation.DBRecordVersion); err != nil {
				return err
			}
		}
	default:
...
func (s *executionStore) insertNewCurrentExecution(
	...
	return newMongoUnavailableErrorf(err, "failed to insert current execution document: %v", err)
}
...
func (s *executionStore) updateExistingCurrentExecution(
	...
	if result.MatchedCount == 0 {
		existing, loadErr := s.loadCurrentExecution(ctx, doc.ShardID, doc.NamespaceID, doc.WorkflowID, chasm.ArchetypeID(doc.ArchetypeID))
		if loadErr != nil {
			return loadErr
		}
		return s.currentWorkflowConflictError(ctx, existing, "current execution update condition failed")
...
func (s *executionStore) ensureRunIDMismatch(
	...
	current, err := s.loadCurrentExecution(ctx, doc.ShardID, doc.NamespaceID, doc.WorkflowID, chasm.ArchetypeID(doc.ArchetypeID))
	if err != nil {
		return err
	}
...
func (s *executionStore) loadCurrentExecution(ctx context.Context, shardID int32, namespaceID, workflowID string, archetypeID chasm.ArchetypeID) (*currentExecutionDocument, error) {
	filter := bson.M{"_id": currentExecutionDocID(namespaceID, workflowID, archetypeID), "shard_id": shardID}
	var doc currentExecutionDocument
	if err := s.currentExecsCol.FindOne(ctx, filter).Decode(&doc); err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			return nil, nil
		}
		return nil, newMongoUnavailableErrorf(err, "failed to load current execution: %v", err)
	}
	return &doc, nil
}
...
func (s *executionStore) currentWorkflowConflictError(
	...
	execDoc, err := s.loadExecutionDocument(ctx, doc.ShardID, doc.NamespaceID, doc.WorkflowID, doc.RunID)
	if err != nil {
		return err
	}
	if execDoc != nil {
...
func (s *executionStore) insertExecutionDocument(ctx context.Context, doc *executionDocument) error {
	...
	if err != nil {
		...
		return newMongoUnavailableErrorf(err, "failed to insert execution document: %v", err)
	}
	return nil
}
	...
	indexes := []struct {
	...
	}{
		{
			...
			model: mongo.IndexModel{
				...
				Options: options.Index().SetName("executions_shard_namespace_workflow_run").SetUnique(true),
			},
		},
	}
...
func (s *executionStore) ensureCurrentExecutionIndexes(ctx context.Context) error {
	...
	indexes := []struct {
		...
		{
			name: "current_executions_shard_namespace_workflow_archetype",
			model: mongo.IndexModel{
				Keys: bson.D{
					{Key: "shard_id", Value: 1},
					{Key: "namespace_id", Value: 1},
				...
				Options: options.Index().SetName("current_executions_shard_namespace_workflow_archetype").SetUnique(true),
			},
		},
		{
...
func (s *executionStore) CreateWorkflowExecution(
	...
	if err != nil {
		...
		if errors.As(err, &conflictErr) {
			existing, loadErr := s.loadCurrentExecution(ctx, request.ShardID, conflictErr.namespaceID, conflictErr.workflowID, conflictErr.archetypeID)
			if loadErr != nil {
				return nil, loadErr
			}
...
func (s *executionStore) executionDocumentToMutableState(doc *executionDocument) *persistence.InternalWorkflowMutableState {
	...
	bufferedEvents := make([]*commonpb.DataBlob, 0, len(doc.BufferedEvents))
	for _, bufferedEvent := range doc.BufferedEvents {
		if blob := dataBlobDocToBlob(bufferedEvent); blob != nil {
			bufferedEvents = append(bufferedEvents, blob)
		}
	}
...
func (s *executionStore) applyConflictResolveWorkflowExecution(
	...
	existing, err := s.loadExecutionDocument(ctx, request.ShardID, resetSnapshot.NamespaceID, resetSnapshot.WorkflowID, runID)
	if err != nil {
		return err
	}
	if existing == nil {
	...
	result, err := s.executionsCol.UpdateOne(ctx, filter, bson.M{"$set": updateSet})
	if err != nil {
		return newMongoUnavailableErrorf(err, "failed to reset execution document: %v", err)
	}
	if result.MatchedCount == 0 {
		return &persistence.WorkflowConditionFailedError{
	...
	case persistence.ConflictResolveWorkflowModeBypassCurrent:
		if err := s.assertNotCurrentExecution(ctx, request.ShardID, resetSnapshot.NamespaceID, resetSnapshot.WorkflowID, request.ArchetypeID, runID); err != nil {
			return err
		}
	case persistence.ConflictResolveWorkflowModeUpdateCurrent:
		...
		if err := s.updateCurrentExecutionForConflict(ctx, request.ShardID, resetSnapshot.NamespaceID, resetSnapshot.WorkflowID, request.ArchetypeID, prevRunID, targetState, targetLastWriteVersion, targetDBRecordVersion); err != nil {
			return err
		}
	default:
...
func (s *executionStore) DeleteWorkflowExecution(
	...
	filter := bson.M{"_id": executionDocID(request.NamespaceID, request.WorkflowID, request.RunID), "shard_id": request.ShardID}
...
func (s *executionStore) GetCurrentExecution(
	...
	doc, err := s.loadCurrentExecution(ctx, request.ShardID, request.NamespaceID, request.WorkflowID, request.ArchetypeID)
	if err != nil {
		return nil, err
	}
	if doc == nil {
...
func (s *executionStore) GetWorkflowExecution(
	...
	doc, err := s.loadExecutionDocument(ctx, request.ShardID, request.NamespaceID, request.WorkflowID, request.RunID)
	if err != nil {
		return nil, err
	}
	if doc == nil {
...
func (s *executionStore) SetWorkflowExecution(
	...
	_, err = s.executeTransaction(ctx, func(sessCtx context.Context) (interface{}, error) {
		...
		existing, err := s.loadExecutionDocument(sessCtx, request.ShardID, snapshot.NamespaceID, snapshot.WorkflowID, runID)
		if err != nil {
			return nil, err
		}
		if existing == nil {
		...
		if err != nil {
			return nil, newMongoUnavailableErrorf(err, "failed to set workflow execution: %v", err)
		}
		if result.MatchedCount == 0 {
...
func (s *executionStore) AddHistoryTasks(
	...
	_, err := s.executeTransaction(ctx, func(sessCtx context.Context) (interface{}, error) {
		...
		return nil, s.insertWorkflowTasks(sessCtx, request.ShardID, request.Tasks)
	})
	return err
}
...
func (s *executionStore) CompleteHistoryTask(
	...
	filter := bson.M{"_id": docID, "shard_id": request.ShardID}
...
func (s *executionStore) AppendHistoryNodes(
	...
	if !request.IsNewBranch || mongo.SessionFromContext(ctx) != nil {
		return s.appendHistoryNodes(ctx, request)
	}

	_, err := s.executeTransaction(ctx, func(txCtx context.Context) (interface{}, error) {
		return nil, s.appendHistoryNodes(txCtx, request)
	})
	return err
}
...
func (s *executionStore) appendHistoryNodes(
	...
	if _, err := s.historyNodesCol.InsertOne(ctx, nodeDoc); err != nil {
		...
		default:
			return newMongoUnavailableErrorf(err, "failed to append history node: %v", err)
		}
	}
	...
	if request.IsNewBranch {
		...
		if _, err := s.historyBranchesCol.UpdateOne(ctx, bson.M{"_id": branchDoc.ID, "shard_id": request.ShardID}, update, opts); err != nil {
			if errors.Is(err, context.DeadlineExceeded) || errors.Is(err, context.Canceled) {
				return &persistence.AppendHistoryTimeoutError{Msg: err.Error()}
			}
			return newMongoUnavailableErrorf(err, "failed to upsert history branch: %v", err)
		}
	}
...
func (s *executionStore) DeleteHistoryNodes(
	...
	treeID := branchInfo.GetTreeId()
	branchID := branchInfo.GetBranchId()
	filter := bson.M{"_id": historyNodeDocID(treeID, branchID, request.NodeID, request.TransactionID), "shard_id": request.ShardID}
...
func (s *executionStore) ForkHistoryBranch(
	...
	if _, err := s.historyBranchesCol.UpdateOne(ctx, bson.M{"_id": branchDoc.ID, "shard_id": request.ShardID}, update, options.Update().SetUpsert(true)); err != nil {
...
func (s *executionStore) DeleteHistoryBranch(
	...
	branchInfo := request.BranchInfo
	filter := bson.M{"_id": historyBranchDocID(branchInfo.GetTreeId(), branchInfo.GetBranchId()), "shard_id": request.ShardID}
...
// assertShardRangeID verifies shard ownership and takes a write lock on the shard document.
// Must be called within the workflow transaction so ownership changes conflict with its writes.
func (s *executionStore) assertShardRangeID(
	...
	result, err := collection.UpdateOne(
		ctx,
		bson.M{"_id": shardID, "range_id": expectedRangeID},
		bson.M{"$set": bson.M{"transaction_lock_id": uuid.NewString()}},
	)
	if err != nil {
		return newMongoUnavailableErrorf(err, "failed to lock shard document: %v", err)
	}
	...
	if result.MatchedCount == 0 {
		return &persistence.ShardOwnershipLostError{
			ShardID: shardID,
			Msg: fmt.Sprintf(
				"Shard ownership lost. Expected RangeID: %v",
				expectedRangeID,
			),
		}
	}
