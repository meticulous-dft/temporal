...
func (s *queueV2Store) EnqueueMessage(
	...
	result, err := s.executeTransaction(ctx, func(txCtx context.Context) (interface{}, error) {
		...
		if _, err := s.messagesCol.InsertOne(txCtx, doc); err != nil {
			...
			return nil, newMongoUnavailableErrorf(err, "failed to insert queue v2 message: %v", err)
		}
...
func (s *queueV2Store) RangeDeleteMessages(
	...
	result, err := s.executeTransaction(ctx, func(txCtx context.Context) (interface{}, error) {
		metaDoc, queueProto, err := s.loadQueueMetadata(txCtx, request.QueueType, request.QueueName)
		if err != nil {
			return nil, err
		}
		...
		if _, err := s.messagesCol.DeleteMany(txCtx, filter); err != nil {
			return nil, newMongoUnavailableErrorf(err, "failed to delete queue v2 messages: %v", err)
		}

		partition.MinMessageId = deleteRange.NewMinMessageID
		if err := s.updateQueueMetadataPayload(txCtx, metaDoc.ID, metaDoc.Version, queueProto); err != nil {
			return nil, err
		}
	...
	response, ok := result.(*persistence.InternalRangeDeleteMessagesResponse)
	if !ok {
		return nil, serviceerror.NewInternal("unexpected range delete transaction result type")
	}
	return response, nil
}
...
func (s *queueV2Store) allocateMessageID(ctx context.Context, queueType persistence.QueueV2Type, queueName string) (int64, error) {
	...
	update := bson.M{
		"$inc": bson.M{"last_message_id": int64(1), "version": int64(1)},
		"$set": bson.M{"updated_at": time.Now().UTC()},
	}

	res, err := s.metadataCol.UpdateOne(ctx, filter, update)
	if err != nil {
		return 0, newMongoUnavailableErrorf(err, "failed to update queue metadata: %v", err)
	}
	...
	if err := s.metadataCol.FindOne(ctx, filter).Decode(&doc); err != nil {
		return 0, newMongoUnavailableErrorf(err, "failed to reload queue metadata: %v", err)
	}
...
func (s *queueV2Store) loadQueueMetadata(
	...
	if err := s.metadataCol.FindOne(ctx, filter).Decode(&doc); err != nil {
		...
		return nil, nil, newMongoUnavailableErrorf(err, "failed to load queue metadata: %v", err)
	}
...
func (s *queueV2Store) updateQueueMetadataPayload(
	ctx context.Context,
	metadataID string,
	...
	result, err := s.metadataCol.UpdateOne(ctx, bson.M{"_id": metadataID, "version": expectedVersion}, update)
	if err != nil {
		return newMongoUnavailableErrorf(err, "failed to update queue metadata payload: %v", err)
	}
	if result.MatchedCount == 0 {
		return &persistence.ConditionFailedError{Msg: "queue metadata version conflict"}
	}
	return nil
}
