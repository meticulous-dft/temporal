...
func TestAssertShardRangeID(t *testing.T) {
	const (
		shardID = int32(7)
		rangeID = int64(11)
	)

	t.Run("locks matching shard", func(t *testing.T) {
		shardCol := newFakeCollection(t)
		shardCol.enqueueUpdateResult(&mongo.UpdateResult{MatchedCount: 1})
		shardCol.enqueueUpdateResult(&mongo.UpdateResult{MatchedCount: 1})
		db := newFakeMongoDatabase(t)
		db.collections[collectionShards] = shardCol
		store := &executionStore{db: db}

		require.NoError(t, store.assertShardRangeID(context.Background(), shardID, rangeID))
		require.NoError(t, store.assertShardRangeID(context.Background(), shardID, rangeID))
		require.Equal(t, []interface{}{
			bson.M{"_id": shardID, "range_id": rangeID},
			bson.M{"_id": shardID, "range_id": rangeID},
		}, shardCol.updateFilters)

		lockIDs := make([]string, 0, len(shardCol.updateUpdates))
		for _, updateValue := range shardCol.updateUpdates {
			update, ok := updateValue.(bson.M)
			require.True(t, ok)
			set, ok := update["$set"].(bson.M)
			require.True(t, ok)
			lockID, ok := set["transaction_lock_id"].(string)
			require.True(t, ok)
			require.NotEmpty(t, lockID)
			lockIDs = append(lockIDs, lockID)
		}
		require.NotEqual(t, lockIDs[0], lockIDs[1])
	})

	t.Run("rejects mismatched range", func(t *testing.T) {
		shardCol := newFakeCollection(t)
		shardCol.enqueueUpdateResult(&mongo.UpdateResult{})
		db := newFakeMongoDatabase(t)
		db.collections[collectionShards] = shardCol
		store := &executionStore{db: db}

		err := store.assertShardRangeID(context.Background(), shardID, rangeID)
...
func TestConflictResolveWorkflowExecution_WithCurrentMutationAndNewWorkflow(t *testing.T) {
	...
	require.Equal(t, int32(1), execCol.findFilters[0].(bson.M)["shard_id"])
	require.Equal(t, int32(1), execCol.findFilters[1].(bson.M)["shard_id"])
	...
	require.Equal(t, int32(1), execCol.updateFilters[0].(bson.M)["shard_id"])
	require.Equal(t, int32(1), execCol.updateFilters[1].(bson.M)["shard_id"])
	...
	require.Equal(t, int32(1), currFilter["shard_id"])
...
func TestUpdateWorkflowExecution_ContinueAsNew(t *testing.T) {
	...
	require.Equal(t, int32(1), filter["shard_id"])
...
func TestGetWorkflowExecution_Success(t *testing.T) {
	...
	require.Equal(t, int32(7), execCol.findFilters[0].(bson.M)["shard_id"])
}
...
func TestGetCurrentExecution_Success(t *testing.T) {
	...
	require.Equal(t, int32(8), currCol.findFilters[0].(bson.M)["shard_id"])
}
...
func TestAppendHistoryNodes_NewBranch(t *testing.T) {
	...
	store := &executionStore{
		transactionalStore: newTransactionalStore(mongoClient),
	...
	require.Len(t, mongoClient.sessions, 1)
	require.Equal(t, 1, mongoClient.sessions[0].withTransactionCalls)
...
func TestGetHistoryTasks_ImmediatePagination(t *testing.T) {
	...
	require.Equal(t, shardID, transferCol.findManyFilters[0].(bson.M)["shard_id"])
...
func (c *fakeCollection) Find(_ context.Context, filter interface{}, _ ...*options.FindOptions) (client.Cursor, error) {
	c.findManyFilters = append(c.findManyFilters, filter)
...
func (c *fakeCollection) InsertMany(_ context.Context, documents []interface{}, _ ...*options.InsertManyOptions) (*mongo.InsertManyResult, error) {
	c.insertManyDocs = append(c.insertManyDocs, documents)
	return nil, nil
}
