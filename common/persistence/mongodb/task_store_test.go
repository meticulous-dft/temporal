...
func (s *TaskStoreSuite) SetupSuite() {
	...
	installMongoTestSchema(s.T(), cfg)
	var err error
...
func (s *TaskStoreSuite) TestFairTaskQueues() {
	...
	remaining, err := fairStore.GetTasks(s.ctx, &persistence.GetTasksRequest{
		NamespaceID:        "ns-fair",
		TaskQueue:          "queue-fair",
		TaskType:           enumspb.TASK_QUEUE_TYPE_WORKFLOW,
		InclusiveMinPass:   1,
	...
	s.Require().NoError(err)
	s.Require().Len(remaining.Tasks, 2)
}
